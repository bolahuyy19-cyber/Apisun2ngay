...
const HISTORY_API = "https://sunwinhuydaixu.onrender.com/api/sunwin/history";
const OPENROUTER_API_KEY = "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";
...
async function fetchHistorySafe() {
  ...
  const latestRaw = raw[0]; // Lấy bản ghi mới nhất trước khi reverse
...
/* ================== TOÀN BỘ LOGIC CỦA BẠN ================== */
/* Giữ nguyên 100% như bạn gửi — KHÔNG chỉnh sửa thuật toán  */
...
function generatePrediction(history, modelPredictionsRef) {
  modelPredictions = modelPredictionsRef || modelPredictions; 
...
async function openRouterPredict(history) {
  try {
    const last10 = history.slice(-10).map(h => h.result).join(", ");
    ...
    const response = await axios.post(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        messages: [
          {
            role: "user",
            content: `Lịch sử gần nhất: ${last10}. Dự đoán tiếp theo là Tài hay Xỉu? Chỉ trả lời "Tài" hoặc "Xỉu".` // Thêm prompt để lấy kết quả chuẩn xác hơn
          }
        ]
      },
      {
...
// Đổi từ /predict thành /
app.get("/", async (req, res) => {
  try {
    const { history, latestRaw } = await fetchHistorySafe();

    const nextSession = latestRaw.Phien + 1;

    const algoPrediction = generatePrediction(history, modelPredictions);
    const aiPrediction = await openRouterPredict(history);

    // Xử lý Dự đoán cuối cùng: Ưu tiên thuật toán, nếu AI trả lời giống thì càng tốt
    let finalAnswer = algoPrediction; 
    if (aiPrediction.includes("Tài") && !aiPrediction.includes("Xỉu") && algoPrediction === "Tài") {
      finalAnswer = "Tài";
    } else if (aiPrediction.includes("Xỉu") && !aiPrediction.includes("Tài") && algoPrediction === "Xỉu") {
      finalAnswer = "Xỉu";
    }

    res.json({
      Ket_qua: latestRaw.Ket_qua,
      Phien: latestRaw.Phien,
      Tong: latestRaw.Tong,
      Xuc_xac_1: latestRaw.Xuc_xac_1,
      Xuc_xac_2: latestRaw.Xuc_xac_2,
      Xuc_xac_3: latestRaw.Xuc_xac_3,
      Phien_hien_tai: nextSession,
      Du_doan_thuat_toan: algoPrediction,
      Du_doan_openrouter: aiPrediction,
      Du_doan_cuoi_cung: finalAnswer 
    });
